...
= 7.6.7 =
* Fix: form column not visible — PHP cache render (lgw_cache_render_table) now also outputs form pips, matching JS renderTable.
* Fix: README.md now packaged inside lgw-division-widget/ folder in release zip.

= 7.6.6 =
* Feature: form guide showing last 5 results per team — on standings table (right-aligned, hidden on small screens) and team modal. Hovering a pip shows result, score and date.

= 7.6.5 =
* Fix: fixture modal shows shots-defeat as L, not D, even when team won 3 rinks.
