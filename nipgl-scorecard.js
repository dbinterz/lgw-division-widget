/* NIPGL Scorecard Submission JS - v5.1 */
(function(){
  'use strict';
  ...
  // ── PIN gate ──────────────────────────────────────────────────────────────────
  var pinSubmitBtn = qs('#nipgl-pin-submit');
  var pinInput     = qs('#nipgl-pin-input');

  function attemptPin() {
    var pin = pinInput ? pinInput.value.trim() : '';
    if (!pin) return;
    if (pinSubmitBtn) setLoading(pinSubmitBtn, 'Enter', true);

    var fd = new FormData();
    fd.append('action', 'nipgl_check_pin');
    ...
    xhr.onload = function() {
      ...
      if (pinSubmitBtn) setLoading(pinSubmitBtn, 'Enter', false);
      if (res.success) {
        qs('#nipgl-pin-gate').style.display  = 'none';
        qs('#nipgl-submit-form').style.display = '';
      } else {
        showStatus(qs('#nipgl-pin-error'), res.data || 'Incorrect PIN', 'error');
      }
    };
    xhr.send(fd);
  }
  ...
  if (pinSubmitBtn) {
    pinSubmitBtn.addEventListener('click', attemptPin);
  }
  if (pinInput) {
    pinInput.addEventListener('keydown', function(e){ if(e.key==='Enter') attemptPin(); });
  }
  ...
  if (photoTrigger) {
    photoTrigger.addEventListener('click', function(){ if(photoInput) photoInput.click(); });
  }
    ...
    parsePhotoBtn.addEventListener('click', function(){
      ...
      xhr.onload = function(){
        ...
        if (res.success) {
          ...
          showStatus(photoStatus, '✅ Scorecard read successfully — please check all fields below.', 'ok');
        } else {
          showStatus(photoStatus, '❌ ' + (res.data || 'Could not read scorecard'), 'error');
        }
      };
  ...
  if (excelInput) {
    excelInput.addEventListener('change', function(){
      if (excelInput.files[0] && parseExcelBtn) parseExcelBtn.style.display = '';
    });
  }
  ...
  if (parseExcelBtn) {
    parseExcelBtn.addEventListener('click', function(){
      ...
      xhr.onload = function(){
        ...
        if (res.success) {
          populateForm(res.data);
          showStatus(excelStatus, '✅ Spreadsheet read — please check all fields below.', 'ok');
        } else {
          showStatus(excelStatus, '❌ ' + (res.data || 'Could not read file'), 'error');
        }
      };
  ...
  function collectForm() {
    var sc = {
      ...
      home_total:  parseFloatOrNull((qs('#sc-home-total')  || {}).value),
      away_total:  parseFloatOrNull((qs('#sc-away-total')  || {}).value),
      home_points: parseFloatOrNull((qs('#sc-home-points') || {}).value),
      away_points: parseFloatOrNull((qs('#sc-away-points') || {}).value),
      rinks: [],
    };
    for (var r = 1; r <= 4; r++) {
      var hp = [], ap = [];
      ...
      sc.rinks.push({
        ...
        home_players: hp,
        away_players: ap,
        home_score: parseFloatOrNull(hs ? hs.value : ''),
        away_score: parseFloatOrNull(as ? as.value : ''),
      });
    }
    return sc;
  }
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', function(){
      ...
      if (!sc.home_team || !sc.away_team) {
        showStatus(saveStatus, '❌ Please enter both home and away team names', 'error');
        return;
      }
      ...
      var fd = new FormData();
      fd.append('action',    'nipgl_save_scorecard');
      fd.append('nonce',     nonce);
      ...
      xhr.onload = function(){
        ...
        var res = JSON.parse(xhr.responseText || '{}');
        if (res.success) {
          showStatus(saveStatus, '✅ ' + res.data.message, 'ok');
          saveBtn.textContent = 'Save Scorecard';
        } else {
          showStatus(saveStatus, '❌ ' + (res.data || 'Save failed'), 'error');
        }
      };
      xhr.onerror = function(){ setLoading(saveBtn, 'Save Scorecard', false); showStatus(saveStatus, '❌ Network error', 'error'); };
      xhr.send(fd);
    });
  }
  ...
  // ── Scorecard display in fixture modal (called from nipgl-widget.js) ──────────
  window.nipglFetchScorecard = function(home, away, date, containerEl) {
    ...
    var url = ajaxUrl + '?action=nipgl_get_scorecard'
    ...
    xhr.onload = function(){
      ...
      if (res.success && res.data) {
        containerEl.innerHTML = nipglRenderScorecard(res.data);
      } else {
  ...
  function nipglRenderScorecard(sc) {
