...
echo "\n== Test 9: manual finals occupant swap (permute the semi-final slots) ==\n";
$champ = unserialize( file_get_contents( __DIR__ . '/fixtures/gchamp-over55-pairs.ser' ) );
$champ['days'][0]['ko_qualifiers'] = array( 'A0', 'B0' );
$champ['days'][1]['ko_qualifiers'] = array( 'A1', 'B1' );
$champ['days'][2]['ko_qualifiers'] = array( 'A2', 'B2' );
$champ['finals_matches'] = lgw_gchamp_build_finals_matches( $champ );
// N=6: idx2 = SF1 (home = seed1 bye A0, away = Winner of QF1),
//      idx3 = SF2 (home = seed2 bye B0, away = Winner of QF2).
eq( "SF1 home is the bye A0", 'A0', $champ['finals_matches'][2]['home'] );
eq( "SF1 away = Winner of QF1", 0, $champ['finals_matches'][2]['away_prev'] );
eq( "SF2 away = Winner of QF2", 1, $champ['finals_matches'][3]['away_prev'] );
// The SF occupant group offers both byes and both QF winners in one list.
$og = lgw_gchamp_finals_occupant_groups( $champ );
eq( "SF group has 4 occupant options (2 byes + 2 QF winners)", 4, count( $og['Semi-final']['options'] ?? array() ) );
check( "SF group is a mixed (has_win) round", ! empty( $og['Semi-final']['has_win'] ) );
check( "QF group is pure-seed (no winner feeds)", empty( $og['Quarter-final']['has_win'] ) );
$GLOBALS['__opt']['lgw_gchamp_nipgl-over55-pairs-2026'] = $champ;

// (a) reroute: send Winner of QF1 (win:0) into SF2 away.
$_POST = array( 'champ_id' => 'nipgl-over55-pairs-2026', 'nonce' => 'x', 'slot' => '3:away', 'token' => 'win:0' );
try { lgw_ajax_gchamp_finals_set_occupant(); check( "set_occupant(reroute) did not error", false ); }
catch ( LGW_JsonDone $j ) { check( "set_occupant(reroute) succeeded", $j->ok === true ); }
$s = $GLOBALS['__opt']['lgw_gchamp_nipgl-over55-pairs-2026'];
eq( "SF2 away now Winner of QF1", 0, $s['finals_matches'][3]['away_prev'] );
eq( "SF1 away swapped to Winner of QF2", 1, $s['finals_matches'][2]['away_prev'] );

// (b) the key ask — put a known bye into a "Winner of QFx" slot: seed1 (A0) → SF1 away.
$GLOBALS['__opt']['lgw_gchamp_nipgl-over55-pairs-2026'] = $s;
$_POST = array( 'champ_id' => 'nipgl-over55-pairs-2026', 'nonce' => 'x', 'slot' => '2:away', 'token' => 'seed:1' );
try { lgw_ajax_gchamp_finals_set_occupant(); check( "set_occupant(bye→winner slot) did not error", false ); }
catch ( LGW_JsonDone $j ) { check( "set_occupant(bye→winner slot) succeeded", $j->ok === true ); }
$s = $GLOBALS['__opt']['lgw_gchamp_nipgl-over55-pairs-2026'];
eq( "SF1 away now holds the bye A0", 'A0', $s['finals_matches'][2]['away'] );
eq( "SF1 away is a seed (bye) slot", 1, $s['finals_matches'][2]['away_seed'] );
check( "displaced winner feed moved to SF1 home", $s['finals_matches'][2]['home_prev'] !== null );
check( "SF1 home no longer a bye", $s['finals_matches'][2]['home_seed'] === null );

// (c) locked once a score exists.
$s['finals_matches'][0]['home_score'] = 21; $s['finals_matches'][0]['away_score'] = 5;
$GLOBALS['__opt']['lgw_gchamp_nipgl-over55-pairs-2026'] = $s;
$_POST = array( 'champ_id' => 'nipgl-over55-pairs-2026', 'nonce' => 'x', 'slot' => '3:away', 'token' => 'win:1' );
try { lgw_ajax_gchamp_finals_set_occupant(); check( "locked swap did not silently pass", false ); }
catch ( LGW_JsonDone $j ) { check( "occupant swap rejected after finals started", $j->ok === false ); }
