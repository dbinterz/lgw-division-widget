...
function nipgl_build_scorecard_pdf($sc, $post_id) {
    $pdf = new NipglPdf();
    $pdf->AddPage();
    ...
    $status    = get_post_meta($post_id, 'nipgl_sc_status',    true) ?: 'pending';
    $con_by    = get_post_meta($post_id, 'nipgl_confirmed_by', true) ?: '';
    $sub_by    = get_post_meta($post_id, 'nipgl_submitted_by', true) ?: '';
    $edited_at = get_post_meta($post_id, 'nipgl_admin_edited', true) ?: '';
    ...
    $pdf->SetXY(15, 15);
    $pdf->Cell($pw, 12, 'NIPGL Match Scorecard', 0, 1, 'C', false);
    ...
    // ── Audit summary ─────────────────────────────────────────────────────────
    $audit_log = get_post_meta($post_id, 'nipgl_audit_log', true) ?: array();
    if (!empty($audit_log)) {
        $y += 4;
    ...
    $pdf->SetXY(15, 285);
    $pdf->Cell($pw, 5, 'Generated ' . date('d M Y H:i') . ' by NIPGL Division Widget', 0, 0, 'R', false);
